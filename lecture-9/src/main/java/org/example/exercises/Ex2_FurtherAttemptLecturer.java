...
public class Ex2_FurtherAttemptLecturer
{
/*
     Lecturer provided the following code in this class.
     Much smoother approach.
     First method (line 60) Splitting the original text into constituent sentences using regex - then add them to a List<String>
     Second method (line 73) search through list made in previous method, extract Strings from list that contain search word

     This approach misses the target slightly
     The Regex from SplitParagraphIntoSentences cuts the first String result off prematurely, inside the brackets containing (i.e. ...)
     This is the same problem I originally faced

    Output of these methods:
    Sentence The most recent official economic and fiscal forecasts, from March 2022, had the government meeting its fiscal targets with a current budget surplus (i.e. contains the word fiscal.
Sentence Under Citi’s central forecast, it would require a fiscal tightening of £62 billion in 2026–27 to stabilise debt as a fraction of national income – so even reversing all of the permanent tax cuts in Mr Kwarteng’s ‘mini-Budget’ would not be enough. contains the word fiscal.
Sentence Higher growth would help – but even if growth turned out to be 0.25 percentage points a year stronger than Citi expects, a fiscal tightening of £41 billion would be required to stabilise debt. contains the word fiscal.
Sentence The government might be inclined to deliver any fiscal tightening through spending cuts. contains the word fiscal.
Sentence While we should hope for better growth, the rationale for an independent OBR is to ensure that politically motivated wishful thinking is not incorporated into economic and fiscal forecasts. contains the word fiscal.
Sentence Recent events have illustrated the importance of a credible strategy and plan for fiscal sustainability. contains the word fiscal.
Sentence Just as financial markets – which ultimately have to lend the money required to plug the gap in the government’s fiscal plans – might be unconvinced by plans underpinned by an assumption of a miraculous uptick in growth, so too might they be unconvinced by plans underpinned by vague promises of public spending cuts far into the future. contains the word fiscal.
     */
    ...
    public static void main(String[] args)
    {
        ...
        List<String> sentenceList = SplitParagraphIntoSentences(ifsOutlook);
    ...
    public static List<String> SplitParagraphIntoSentences(String paragraph)
    {
        //Poached from https://stackoverflow.com/questions/21430447/how-to-split-paragraphs-into-sentences
