...
public class Ex2_ExtraStuff
{
    ...
    public static void main(String[] args)
    {
        ...
        // Extra stuff to continue from exercise 2 - it now asks about case sensitivity and also identifies where in the sentence the word was found
        String originalSentence = readString("Enter a sentence: \n");
        List<String> sentenceAsArrayList = setStringArrayListFromSentence(originalSentence);

        String searchWord = readString("Enter a word to search for: \n");

        boolean caseMustMatch = requireCaseSensitivity("Do you want to be tricky and demand the words match case?");

        int foundCount = searchForWordReturnCount(sentenceAsArrayList, sentenceAsArrayList.size(), searchWord, caseMustMatch);  // case checking
        declareFoundCount(foundCount, searchWord, originalSentence);

        List<Integer> foundPositions = searchForWordReturnPositionsArrList(sentenceAsArrayList, sentenceAsArrayList.size(), searchWord, caseMustMatch); // word position checking
        declareFoundPositions(foundPositions, searchWord, originalSentence);

        // Extra code in main method to test out the additional work looking at multi-sentence strings in comment block below
        /*
        String originalSentence = ifsOutlook;
        extractSentencesContainingWord(originalSentence, "fiscal");

        System.out.printf("%s", "My solution: \n");
        extractSentencesContainingWord("Hello my name is Arthur.", "Arthur");
        extractSentencesContainingWord("Hello my name is Arthur. uihiuhiuhi", "Arthur");
        extractSentencesContainingWord("Hello my name is Arthur (i.e. it is not some other name).", "Arthur");


        System.out.printf("%s", "Combined team solution: \n");
        teamExtractSentencesContainingWord("Hello my name is Arthur.", "Arthur");
        teamExtractSentencesContainingWord("Hello my name is Arthur. uihiuhiuhi", "Arthur");
        teamExtractSentencesContainingWord("Hello my name is Arthur (i.e. it is not some other name).", "Arthur");

        System.out.printf("%s", "Third attempt: \n");
        thirdAttempt("Hello my name is Arthur.", "Arthur");
        thirdAttempt("Hello my name is Arthur. uihiuhiuhi", "Arthur");
        thirdAttempt("Hello my name is Arthur (i.e. it is not some other name).", "Arthur");
        thirdAttempt(ifsOutlook, "fiscal");
        */
    ...
    public static List<String> setStringArrayListFromSentence(String originalSentence)
    {
        return Arrays.stream(originalSentence.split(" ")).toList();
    }


    //find the word being searched for and return the count of times it is found, takes additional parameter to check for case-matching
    public static int searchForWordReturnCount(List<String> sentenceAsArrayList, int listSize, String searchWord, boolean caseMustMatch)
    {
        int countFound = 0;
        ...
        if (caseMustMatch)
        {
            ...
            {
                if (sentenceAsArrayList.get(i).equals(searchWord))
                {
                    countFound++;
                }
            }
        ...
        else
        {
            for (int i = 0; i < listSize; i++)
            {
                if (sentenceAsArrayList.get(i).equalsIgnoreCase(searchWord))
                {
                    countFound++;
                }
            }
    ...
    //find the word being searched for and return the sentence position it was found in as an array
    public static List<Integer> searchForWordReturnPositionsArrList(List<String> sentenceArrayList, int listSize, String searchWord, boolean caseMustMatch)
    {
        ...
        if (caseMustMatch)
        {
            for (int i = 0; i < listSize; i++)
            {
                if (sentenceArrayList.get(i).equals(searchWord))
                {
        ...
        else
        {
            for (int i = 0; i < listSize; i++)
            {
                if (sentenceArrayList.get(i).equalsIgnoreCase(searchWord))
                {
                    foundIndexes.add(i + 1);
                }
            }
    ...
    // print to console the positions the word was found in
    public static void declareFoundPositions(List<Integer> foundIndexes, String searchWord, String originalSentence)
    {
        System.out.printf("The original sentence was: \n%s\n", originalSentence);

        String positionsArrAsString = "";

        for (int i = 0; i < foundIndexes.size(); i++)
        {
            if (i == foundIndexes.size() - 1)
            {
                positionsArrAsString = positionsArrAsString.concat(foundIndexes.get(i) + ".");
            }
            else
            {
                positionsArrAsString = positionsArrAsString.concat(foundIndexes.get(i) + ", ");
        ...
        System.out.printf("The word '%s' was found in positions: \n%s\n", searchWord, positionsArrAsString);
    }


    //establish whether to be picky and only look for words which match cases
    public static boolean requireCaseSensitivity(String prompt)
    {
        System.out.printf("%s\n", prompt);
        return stdin.nextLine().equalsIgnoreCase("yes");
    }

    // The following methods are attempts to extract entire sentences from a multi-sentence string, based on searching for a specific word
    // For this test I grabbed a statement from the IFS and search for the word "fiscal"
    // Lines 35 to 53 test these methods using a simple string, originally identified an issue where it would stop at full stops within brackets, but that was resolved
    // Yet to solve the issue of not stopping at a fullstop outside the brackets though

    //My initial attempt
    public static void extractSentencesContainingWord(String originalText, String searchWord)
    {
        Pattern regexPattern = Pattern.compile("[^.]*\\b(" + searchWord + ")\\b[^.]*[.]");
        Matcher matcher = regexPattern.matcher(originalText);
        String sentenceWithinInput = "";
        while (matcher.find())
        {
            sentenceWithinInput = matcher.group(0);
            System.out.printf("%s\n", sentenceWithinInput);
        }
    }

    // Attempt following asking squad for help
    public static void teamExtractSentencesContainingWord(String originalText, String searchWord)
    {
        final String regex = "^.*(" + searchWord + ").*\\.";
    ...
    public static void thirdAttempt(String originalText, String searchWord)
    {
        final String regex = "^\s.*[(\s|]" + searchWord + "[)\s].*\\.";
    ...
    // String used to test extraction of entire sentences from a larger String
    // quoted from: https://ifs.org.uk/publications/outlook-public-finances
    public static String ifsOutlook = "Key findings" +
